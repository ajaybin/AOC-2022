...
void get_steps_to(node* destination) {
	int steps = 0;
...
int main() {
	...
	for (size_t _row = 0; _row < num_rows; _row++) {
		for (size_t _col = 0; _col < num_cols; _col++) {
			...
			if (upsy != NULL && (mynode->height + 1 >= upsy->height)) {
				edge* myedge = new edge(mynode, upsy);
			}
			if (down != NULL && (mynode->height + 1 >= down->height)) {
				edge* myedge = new edge(mynode, down);
			}
			if (left != NULL && (mynode->height + 1 >= left->height)) {
				edge* myedge = new edge(mynode, left);
			}
			if (rite != NULL && (mynode->height + 1 >= rite->height)) {
				edge* myedge = new edge(mynode, rite);
			}
		}
	}
	node* start_node = get_node_at_position(start);
	node* end_node = get_node_at_position(end);
	start_node->distance_from_start = 0;
	dijkstras();
	get_steps_to(end_node);
	cout << "END";
}
